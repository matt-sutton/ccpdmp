  ...
  for (i in 1:ndim) {
    for (j in 1:ndim) {
      if (i == j & nsamples > 0) {
        ...
        for (r in 2:nres) {
          lines(density(samples[[r]]$x[coords[i], ],
      ...
      if (i != j) {
        xrange <- range(sapply(1:nres, function(rs) range(c(samples[[rs]]$x[coords[i],
        ], list_pdmp[[rs]]$positions[coords[i], inds]))))
        yrange <- range(sapply(1:nres, function(rs) range(c(samples[[rs]]$x[coords[j],
        ...
        plot(list_pdmp[[1]]$positions[coords[i], inds],
             ...
             main = "", col = colres[1])
        points(samples[[1]]$x[coords[i], ], samples[[1]]$x[coords[j],
        ], col = colres[1], pch = pch)
        for (r in 2:nres) {
          lines(list_pdmp[[r]]$positions[coords[i], inds],
                list_pdmp[[r]]$positions[coords[j], inds],
                col = colres[r])
          points(samples[[r]]$x[coords[i], ], samples[[r]]$x[coords[j],
